...
(block! "Vim Emulation"
        ...
        ;; Optional: Make evil-mode the default for all buffers
        (setq evil-default-state 'normal)
        
        ;; Optional: Use C-z to toggle between Emacs state and Normal state
        (define-key evil-normal-state-map (kbd "C-z") 'evil-emacs-state)
        (define-key evil-emacs-state-map (kbd "C-z") 'evil-normal-state))

(block! "ABNT2 Keyboard Layout"
        (defun chin/setup-abnt2-keyboard ()
          "Setup ABNT2 keyboard layout for Emacs."
          (interactive)
          ;; Define key mappings for ABNT2-specific keys
          (global-set-key (kbd "§") "`")
          (global-set-key (kbd "º") "º")
          (global-set-key (kbd "ª") "ª")
          (global-set-key (kbd "´") "'")
          (global-set-key (kbd "¨") "\"")
          (global-set-key (kbd "ç") "ç")
          
          ;; Handle dead keys
          (require 'iso-transl)
          
          (message "ABNT2 keyboard layout configured."))

        ;; Optionally, you can call this function automatically on startup
        ;; (chin/setup-abnt2-keyboard)
        
        ;; Or bind it to a key for easy toggling
        (global-set-key (kbd "<f8>") 'chin/setup-abnt2-keyboard))

(use-package evil
  :ensure t
  :init
  (setq evil-want-C-u-scroll t) ;; Habilita o scroll com C-u
  (setq evil-want-C-i-jump nil) ;; Desabilita o comportamento de jump com C-i
  :config
  (evil-mode 1))
...
;; Opções adicionais para melhorar a experiência com o evil-mode
...
;; Configurações visuais para o modo normal e visual
(setq evil-normal-state-cursor '("green" box))
(setq evil-visual-state-cursor '("orange" box))

;; Inicia o Evil no modo Emacs para os seguintes modos
(evil-set-initial-state 'eshell-mode 'emacs)
(evil-set-initial-state 'term-mode 'emacs)
(evil-set-initial-state 'shell-mode 'emacs)

;; Habilita o modo Evil em todos os buffers
(global-evil-mode 1)

;; Configurações para teclado ABNT2 no Emacs GUI

;; Define o padrão de entrada de teclado para ABNT2
(set-input-method "portuguese-prefix")

;; Configurações adicionais para teclas específicas
(setq default-input-method "portuguese-prefix")

;; Mapeamento de teclas para caracteres especiais
(define-key key-translation-map (kbd "C-ç") (kbd "C-~"))
(define-key key-translation-map (kbd "C-~") (kbd "C-ç"))
(define-key key-translation-map (kbd "C-;") (kbd "C-`"))

;; Configurações específicas para acentuação
(require 'iso-transl)
